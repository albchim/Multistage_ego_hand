...
from .transforms import transformManoParamsToCam
...
def augmentation_(img, bbox, joint_coord, joint_valid, hand_type, mode, joint_type):
    img = img.copy(); 
    joint_coord = joint_coord.copy(); 
    #hand_type = hand_type.copy();
...
def get_bbox(joint_img, joint_valid):
    ...
    bbox = torch.tensor([xmin, ymin, xmax-xmin, ymax-ymin])#.astype(np.float32)
    return bbox
...
def process_bbox(bbox):#, original_img_shape):
    ...
    c_x = bbox[0] + w/2.
    c_y = bbox[1] + h/2.
    aspect_ratio = 1#original_img_shape[1]/original_img_shape[0]
    if w > aspect_ratio * h:
        h = w / aspect_ratio
...
def compute_hand_area(joints):
    x = joints[:,0]; y = joints[:,1];
    xmin = torch.min(x); ymin = torch.min(y); xmax = torch.max(x); ymax = torch.max(y);
    return torch.abs(xmax-xmin)*torch.abs(ymax-ymin)
