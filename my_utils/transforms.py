...
mano_layer = {
    'right' : ManoLayer(
                    ncomps=45, # only used if use_pca is set to True
                    center_idx=9,
                    side='right',
...
smpl_data_hands = {}
smpl_data_hands['left'] = loadPickleData(osp.join(smplx_path, 'MANO_LEFT.pkl'))
smpl_data_hands['right'] = loadPickleData(osp.join(smplx_path, 'MANO_RIGHT.pkl'))

def compute_rel_trans(left_pose, right_pose):
    return right_pose - left_pose

def cam2pixel(cam_coord, f, c):
    if len(cam_coord.shape) == 3:
        x = cam_coord[:, :, 0] / (cam_coord[:, :, 2] + 1e-8) * f[0,0] + c[0,0]
        y = cam_coord[:, :, 1] / (cam_coord[:, :, 2] + 1e-8) * f[0,1] + c[0,1]
        z = cam_coord[:, :, 2]
        img_coord = torch.cat((x[:, :,None], y[:, :,None], z[:, :,None]),-1)
    elif len(cam_coord.shape)==2:
...
def getHandJointLocs(betas, hand_type):
    ...
    smpl_data['v_shaped'] = mano_layer[hand_type].th_shapedirs.numpy().dot(betas) + smpl_data['v_template']
...
def transformManoParamsToCam(mano_pose, mano_trans, mano_shape, cam_rot, cam_trans, hand_type):
    if type(mano_pose)==torch.Tensor:
        device = mano_pose.device
        mano_pose = mano_pose.clone().numpy()
    if type(mano_trans)==torch.Tensor:
        mano_trans = mano_trans.clone().numpy()
    ...
    return torch.tensor(mano_pose_trans, device=device, dtype=torch.float32), torch.tensor(TAbs, device=device, dtype=torch.float32), mano_shape
